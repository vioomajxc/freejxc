...
// +----------------------------------------------------------------------
// | Author: LotusAdmin <whndeweilai@163.com>  <qq 610176732>
// +----------------------------------------------------------------------
...
function sp_update_site_configs($db, $table_prefix)
{
    ...
    $email           = input("post.manager_email");
    $siteurl         = input("post.siteurl");
    $seo_keywords    = input("post.sitekeywords");
    $seo_description = input("post.siteinfo");
    $site_options    = <<<helllo
            {
            		"site_name":"$sitename",
            		"site_host":"$siteurl",
...
function sp_create_config($config, $authcode)
{
    if (is_array($config)) {
        ...
        //写入应用配置文件
        if (file_put_contents(LOTUS_ROOT.'data/database.php', $conf)) {
...
function sp_create_db_config($config)
{
    if (is_array($config)) {
        ...
        try {
            $confDir = LOTUS_ROOT . 'data';
            if (!file_exists($confDir)) {
                mkdir($confDir, 0777, true);
            }
            file_put_contents(LOTUS_ROOT . 'data/database.php', $conf);
        } catch (\Exception $e) {
            return false;
