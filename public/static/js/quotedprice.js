var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			false ? (i = '<a class="ui-btn" id="audit">审核</a>', j = '<a class="ui-btn" id="reAudit">反审核</a>') : this.$_checkName.parent().hide();
		...
		setSaleByContact: function(a) {
			var b = this;
			b.salesCombo && Public.ajaxGet("../scm/invQuotedPrice/findNearSaEmp?action=findNearSaEmp", {
				buid: a.id
			}, function(a) {
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.ajaxPost("../scm/invQuotedPrice/add?action=add", {
				...
				var c = $(this),
					d = THISPAGE.getPostData();
				d && c.ajaxPost("../scm/invQuotedPrice/addNew?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
					if (200 === b.status) {
				...
				if (b.preventDefault(), Business.verifyRight("SA_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invQuotedPrice/updateInvSa?action=updateInvSa", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("SA_CHECK")) {
					...
					d && c.ajaxPost("../scm/invQuotedPrice/checkInvSa?action=checkInvSa", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("SA_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/invQuotedPrice/rsBatchCheckInvSa?action=rsBatchCheckInvSa", {
						id: originalData.id
					}, function(b) {
				...
				a.preventDefault(), Business.verifyRight("SA_PRINT") && Public.print({
					title: "销货单列表",
					$grid: $("#grid"),
					pdf: "../scm/invQuotedPrice/toPdf?action=toPdf",
					billType: 10201,
					filterConditions: {
						id: originalData.id
					}
				...
				$.dialog({
					...
					data: {
						url: "../scm/invQuotedPrice/findUnhxList?action=findUnhxList&buId=" + originalData.buId + "&id=" + originalData.id
					},
					lock: !0,
					ok: function() {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invQuotedPrice/update?action=update", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invQuotedPrice/update?action=update", {
					id: a.idList[a.idPostion]
				}, function(a) {
...
$(function() {
	if (urlParam.id) {
		if (!hasLoaded) {
			...
			}) : Public.ajaxGet("../scm/invQuotedPrice/update?action=update", {
				id: urlParam.id
			}, function(b) {
